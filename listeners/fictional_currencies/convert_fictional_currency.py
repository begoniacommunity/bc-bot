...
def convert_fictional_currency(update: Update, context: CallbackContext):
    ...
    for currency_code, amount, start_index, end_index in matches:
        chance = react_chances.get(currency_code, 1)
        if random.random() < chance:
            if (currency_code, amount) not in processed_amounts:
                processed_amounts.add((currency_code, amount))
                rates = get_fictional_rates(currency_code)
                if rates is not None:
                    response += f'{fictional_currency_emojis.get(currency_code, "")}{amount:.2f} {currency_code}:\n\n'
                    for currency in rates.keys():
                        rate = rates[currency]
                        response += f'{fictional_currency_emojis.get(currency, "")}{rate * Decimal(str(amount)):.2f} {currency}\n'
